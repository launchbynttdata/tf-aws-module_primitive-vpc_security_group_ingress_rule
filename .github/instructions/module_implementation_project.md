...
- **security_group_id** _(string, required)_ — Target SG for the ingress rule.
- **ip_protocol** _(string, required)_ — IANA protocol name or number (e.g., `tcp`, `udp`, `icmp`, `-1`).
- **from_port** _(number, conditional)_ — Required for port-based protocols.
- **to_port** _(number, conditional)_ — Same rule as `from_port`.
- **cidr_ipv4** _(string, optional)_ — Single IPv4 CIDR for source.
- **cidr_ipv6** _(string, optional)_ — Single IPv6 CIDR for source.
- **prefix_list_id** _(string, optional)_ — Source via managed prefix list.
- **referenced_security_group_id** _(string, optional)_ — Source SG (intra-/inter-SG).
- **description** _(string, optional)_ — Rule description.
...
- Use **lightweight** validation only. Validations must be either:
  - **Precise and complete** - covering all edge cases accurately, OR
  - **Removed** - letting the AWS provider handle validation entirely
- Module validations implemented:
  - **Source mutual exclusivity**: Ensure exactly one of `cidr_ipv4`, `cidr_ipv6`, `prefix_list_id`, or `referenced_security_group_id` is set
  - **Port requirements for TCP/UDP**: Validate that both `from_port` and `to_port` are specified for TCP and UDP protocols
  - **ID format validations**: Basic regex validation for security group IDs and prefix list IDs
- Validations **NOT** implemented (deferred to AWS provider):
  - Port range values and validity (too protocol-specific to validate accurately)
  - Port range ordering (from_port <= to_port)
  - Protocol-specific port requirements (e.g., -1 only valid for ICMP)
- Provide clear, succinct error messages; let the provider surface deep validation errors

### 3.3 Tagging Strategy

Follow the canonical tagging pattern from the reference modules:
...
- Always set `provisioner = "Terraform"` as a default tag
- Merge user-provided tags via `var.tags`
- Apply merged tags to the resource

### 3.4 Idempotence & Drift
...
### Q7: What tagging strategy should we use for module resources?

**Response:** Follow the **canonical tagging pattern** from reference modules (e.g., [tf-aws-module_primitive-lambda_layer](https://github.com/launchbynttdata/tf-aws-module_primitive-lambda_layer/blob/main/locals.tf#L13-L19)):
...
This ensures consistent tagging across all primitive modules with:

- `provisioner = "Terraform"` as the default tag
- User-provided tags merged in via `var.tags`

### Q8: How should we handle variable validation accuracy?

**Response:** Variable validations must be **either precise or removed**. The validation policy:

- ✅ **Implement** validations that can be 100% accurate (e.g., ID format checks, source mutual exclusivity)
- ❌ **Remove** validations that are imprecise or protocol-dependent (e.g., port range values, which vary by protocol)
- Let the AWS provider handle complex, context-sensitive validations

**Specific decisions:**

- ✅ Keep: Security group ID format validation (`^sg-[a-f0-9]+$`)
- ✅ Keep: Prefix list ID format validation (`^pl-[a-f0-9]+$`)
- ✅ Keep: Source mutual exclusivity check
- ✅ Keep: TCP/UDP port requirement check
- ❌ Remove: Port range value validation (can't distinguish TCP 1-65535 vs ICMP -1)
- ❌ Remove: Port range ordering validation (from_port <= to_port)

This prevents false positives where validation incorrectly rejects valid configurations (e.g., allowing -1 for TCP when it's only valid for ICMP).
...
- **Removed imprecise validations**: Eliminated port range value and ordering validations that couldn't accurately handle protocol-specific requirements (e.g., -1 is valid for ICMP but not TCP).
- **Validation rules implemented**:
  ...
  - ID format validations: Security group IDs and prefix list IDs
- **Validation rules removed** (deferred to AWS provider):
  - Port range value validation (protocol-dependent)
  - Port range ordering validation (from_port <= to_port)

### 10.2 Tagging Implementation

- **Applied canonical tagging pattern**: Implemented the standard tagging strategy from reference modules
- **Default tags**: Set `provisioner = "Terraform"` as the default tag
- **Tag merging**: User-provided tags via `var.tags` are merged with defaults using `merge(local.default_tags, var.tags)`

### 10.3 Test Framework Adjustments
...
- ✅ Canonical tagging strategy implemented (provisioner = "Terraform" + user tags).
- ✅ Variable validations are either precise or removed (no imprecise validations).
